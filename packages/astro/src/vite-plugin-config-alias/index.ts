...
/**
 * Regex matching CSS @import, @reference, and @config statements with the specifier in capture group 1.
 * Handles: @import "...", @import url("..."), @reference "...", @config "..."
 */
const cssImportRE = /@(?:import|reference|config)\s+(?:url\(\s*)?['"]([^'"]+)['"]\s*\)?/g;
...
/**
 * Rewrite tsconfig path aliases in CSS source text for @import, @reference, @config, and url() references.
 * Returns the rewritten CSS if any aliases were resolved, or null if no changes were made.
 */
export function resolveCssAliases(code: string, configAlias: Alias[]): string | null {
	...
	// Rewrite @import / @reference / @config aliases
	if (/@(?:import|reference|config)\s/.test(result)) {
